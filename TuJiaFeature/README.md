...
1. **经营驾驶舱**（`/dashboard`）：侧栏单一入口；页内 **Tab** 分为 **市场总览**（KPI、趋势、热力/榜单、预警）、**商圈名录**（`/api/analysis/districts` 聚合表 + 下钻房源）、**设施溢价**（`/api/analysis/facility-premium`，含口径说明）。旧路径 **`/analysis`** 会 **301 式重定向** 至 `/dashboard?tab=districts`（书签与外链仍可用）。  
2. **智能定价预测**：日级 XGBoost（锚定日与 14 天曲线）  
3. **房源推荐**：首页「智能推荐」走 **`/api/home/recommendations`**（SQL+场景/设施重排）；「个性化推荐」页走 **`/api/recommend`**（条件匹配为主，见后端 `RECOMMENDATION_ONLINE_BEHAVIOR.md`）  
4. **房源列表**：紧凑筛选；关键词搜索（标题/行政区/商圈，参数 `keyword`）；登录用户可选「按偏好排序」（`sort_by=personalized`，**规则区域重排**，原理见后端 [`docs/LISTINGS_PERSONALIZED_SORT.md`](../Tujia-backend/docs/LISTINGS_PERSONALIZED_SORT.md)）；详情页会写入浏览历史（见后端 PRD 1.4）  
