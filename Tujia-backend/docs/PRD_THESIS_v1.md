...
**（实现迭代说明 2026-04）** 图中「商圈分析」在前端已与「数据大屏 Dashboard」合并为侧栏单一入口 **经营驾驶舱**（`/dashboard`）下的子 Tab；商圈名录、设施溢价等仍由 **`/api/analysis/*`** 提供。旧路由 `/analysis` 重定向至 `/dashboard?tab=districts`。
...
- **经营驾驶舱 — 分析类 Tab**（原独立「商圈分析」页；现 `/dashboard?tab=districts`、`tab=facilities` 等）：
  - 价格热力图：基于地图的价格分布可视化
  - 供需关系图：商圈饱和度分析
  - 设施溢价排行榜
