...
- [ ] **线上定价 API**依赖日级模型：运行 `python scripts/train_model_daily_mysql.py` 后，`models/` 下存在 `xgboost_price_daily_model.pkl`、`feature_names_daily.json` 及训练导出的 encoder/统计文件（详见脚本 docstring）  
- [ ] `RECOMMENDER_PATH` / `listing_similarity_*.npz` 存在（推荐）；`MODEL_PATH` 指向日级 `xgboost_price_daily_model.pkl`（与 `.env.example` 一致即可）  
...
│   └── services/                 # 业务逻辑
│       ├── daily_price_service.py  # 日级 XGBoost 定价加载与 14 天预测
│       ├── model_manager.py        # 推荐相似度矩阵
│       └── recommender.py          # 推荐服务
├── scripts/                      # 脚本工具
│   ├── train_model_daily_mysql.py # 日级定价训练（主路径）
│   ├── build_recommendation_model.py  # 房源相似度矩阵（内容为主）
│   └── data_processing/         # 数据处理
│       └── data_import.py
├── models/                       # 预训练模型文件
│   ├── xgboost_price_daily_model.pkl（及 feature_names_daily.json 等，由 train_model_daily_mysql 生成）
...
# 模型路径（定价为日级 pkl；推荐为相似度矩阵）
MODEL_PATH=models/xgboost_price_daily_model.pkl
...
### 数据分析模块（`/api/analysis`，前端现挂在经营驾驶舱 Tab）

| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/api/analysis/districts` | 商圈名录聚合（`district` + `trade_area`） |
| GET | `/api/analysis/facility-premium` | 设施溢价（标签分组均价差，**非因果**，见 `BACKEND_API_SPEC`） |
| GET | `/api/analysis/price-distribution` | 价格区间分布（Hive 优先，否则 MySQL） |
| GET | `/api/analysis/price-opportunities` | 价格洼地候选（`items` + `methodology`） |
| GET | `/api/analysis/roi-ranking` | 行政区 ROI 排行（`{ data, field_glossary }`） |

> 下列路径**未在 OpenAPI 注册**，属旧稿占位，勿与实现对齐：`district-stats`、`price-trend/*`、`facility-impact`、`seasonal-effect/*`、`district-comparison`。
...
产物写入 `models/xgboost_price_daily_model.pkl`、`feature_names_daily.json` 等，详见脚本顶部说明。
